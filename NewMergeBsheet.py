...
# Khai bao cac bien
path = os.getcwd() + '\BSheet' # lay duong dan cua file nay + '' (trong ngoac la cai thu muc hien tai)
...
const.columns = range(0,121)
headerlist.columns = range(0,121)
const = pd.concat([headerlist,const])
ErrorTicker = pd.DataFrame(ErrorTicker)
ErrorTicker.to_csv('NewErrorTickerBSheet.csv',encoding='utf-8-sig',index=False, header= False)
const.to_csv('NewBSheetMetgeData.csv', encoding='utf-8-sig',index=False, header= False)
